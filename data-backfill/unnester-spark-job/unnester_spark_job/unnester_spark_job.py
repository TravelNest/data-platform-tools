...
def unnester_worker(client, in_q: Queue, out_q: Queue):
    ...
    def _await_job_completion(client, task: UnnestTask):
        while True:
            task.status = (
                client.get_job_run(JobName=UNNESTER_NAME, RunId=task.task_id)
                .get("JobRun")
                .get("JobRunState")
            )
            logging.info(
                f"Checking unnester tasks {task.task_id}, partitions: {task.partition_str}, status: {task.status}"
            )
...
def get_max_concurrent_runs(client) -> int:
    return client.get_job(JobName=UNNESTER_NAME)["Job"]["ExecutionProperty"][
        "MaxConcurrentRuns"
    ]


def spawn_unnester_workers(client, max_runs: int, in_q: Queue, out_q: Queue):
    for _ in range(0, max_runs):
        threading.Thread(
            target=unnester_worker,
            args=(
                client,
                in_q,
                out_q,
...
def parse_config(args: List[Any]):  # pragma: no cover
    ...
    CLI.add_argument("--refresh", choices=["true", "false"], default="true")
    ...
    config.refresh = True if config.refresh == "true" else False
    return config
